...
// This code implemented techniques include the Local vairables, loop unrolling, and Opreator strength reduction.
...
int main(int argc, char **argv)
{
	...
	register double p_temp;
        p_temp = i*invi*invpi;        
        //use 32 iterations
        cordic((p_temp*MUL), &s, &c, 32);
        //these values should be nearly equal
        printf("%f : %f\n", s/MUL, sin(p_temp));
	
	p_temp = (i+1)*invi*invpi;        
       cordic((p_temp*MUL), &s, &c, 32);
       printf("%f : %f\n", s/MUL, sin(p_temp));
